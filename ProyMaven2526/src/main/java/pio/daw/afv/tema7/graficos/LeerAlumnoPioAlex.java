...
public class LeerAlumnoPioAlex {
	...
	public static void main(String[] args) {
		// Crear marco general
		frame = new JFrame("Datos del alumno");
	...
	private static void ponerComponentes(JPanel panel) {
		...
		btnCreate.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				AlumnoPioAlex al = new AlumnoPioAlex(fieldDNI.getText(), fieldName.getText(), fieldApellido.getText(),
						fielSecondApellido1.getText(), fieldNia.getText(), fieldCiclo.getText(), fieldCurso.getText());
				dataInsert(al);
				JOptionPane.showMessageDialog(panel, "Alumno " + fieldName.getText() + " creado satisfactoriamente!");
				frame.dispose(); // <-- AQUÍ: cierra la ventana
	...
	public static void dataInsert(AlumnoPioAlex al) {
		...
		try (ObjectOutputStream salida = new ObjectOutputStream(new FileOutputStream(RUTA_FICH + "alumnosAlex.dat"))) {
